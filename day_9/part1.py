...
def main(filename: str) -> None:
    ...
    for line in lines:
        ...
        layers[-1].append(Node(value=0, left_child=layers[-2][-1]))
        for index in range(len(layers) - 2, -1, -1):
            prev_layer_last_node = layers[index + 1][-1]
            assert prev_layer_last_node.left_child is not None
            new_node = Node(prev_layer_last_node.left_child.value + prev_layer_last_node.value)
            new_node.parent = prev_layer_last_node
            if index > 0:
                new_node.left_child = layers[index - 1][-1]
            prev_layer_last_node.right_child = new_node
            layers[index].append(new_node)

        longest = len(str(layers[0]))
        for layer in layers:
            print(f"{str(layer):^{longest}s}")
        acc += layers[0][-1].value
    print(acc)
