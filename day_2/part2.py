...
class Game:
    ...
    def fewest_possible(self) -> BallCount:
        result = BallCount()
        for reveal in self.reveals:
            result.red = max(result.red, reveal.red)
            result.green = max(result.green, reveal.green)
            result.blue = max(result.blue, reveal.blue)
        return result
...
def main(filename: str) -> None:
    ...
    for line in lines:
        ...
        fewest = game.fewest_possible()
        power = fewest.power()
        print(f"Fewest: {fewest} - {power} - {game}")
        acc += power
    print(acc)
